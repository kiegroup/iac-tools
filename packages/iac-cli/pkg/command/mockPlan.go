...
func getWorkflowTemplate() (workflowJsonByte []byte, err error) {
	...
	if err != nil {
		fmt.Println("ERROR: marshaling the plan json file.")
	}
	return
}

func CreatePlan(planFilePath string) (err error) {
	planFileData, err := getWorkflowTemplate()
	if err != nil {
		return err
	}

	err = ioutil.WriteFile(planFilePath, planFileData, 0644)
	if err != nil {
		fmt.Println("ERROR: writing the plan file.")
		return err
	}
